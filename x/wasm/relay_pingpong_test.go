...
import (
	...
	wasmibctesting "github.com/ChronicNetwork/chtd/x/wasm/ibctesting"
	wasmkeeper "github.com/ChronicNetwork/chtd/x/wasm/keeper"
	"github.com/ChronicNetwork/chtd/x/wasm/keeper/wasmtesting"
	wasmtypes "github.com/ChronicNetwork/chtd/x/wasm/types"
)
...
func (p player) QueryState(key []byte) uint64 {
	raw := chtd.NewTestSupport(p.t, p.chain.App).WasmKeeper().QueryRaw(p.chain.GetContext(), p.contractAddr, key)
