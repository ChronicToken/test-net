...
[![CircleCI](https://circleci.com/gh/ChronicNetwork/chtd/tree/master.svg?style=shield)](https://circleci.com/gh/ChronicNetwork/chtd/tree/master)
[![codecov](https://codecov.io/gh/ChronicNetwork/chtd/branch/master/graph/badge.svg)](https://codecov.io/gh/ChronicNetwork/chtd)
[![Go Report Card](https://goreportcard.com/badge/github.com/ChronicNetwork/chtd)](https://goreportcard.com/report/github.com/ChronicNetwork/chtd)
[![license](https://img.shields.io/github/license/ChronicNetwork/chtd.svg)](https://github.com/ChronicNetwork/chtd/blob/master/LICENSE)
[![LoC](https://tokei.rs/b1/github/ChronicNetwork/chtd)](https://github.com/ChronicNetwork/chtd)
<!-- [![GolangCI](https://golangci.com/badges/github.com/ChronicNetwork/chtd.svg)](https://golangci.com/r/github.com/ChronicNetwork/chtd) -->

This repository hosts `chtd`, the first implementation of a cosmos zone with wasm smart contracts enabled.

This code was forked from the `cosmos/gaia` repository as a basis and then we added `x/wasm` and cleaned up 
many gaia-specific files. However, the `chtd` binary should function just like `gaiad` except for the
...
a relatively minor change to chtd but would currently require a fork. Please open in issue if this is important for 
your use case.
...
make test
```
if you are using a linux without X or headless linux, look at [this article](https://ahelpme.com/linux/dbusexception-could-not-get-owner-of-name-org-freedesktop-secrets-no-such-name) or [#31](https://github.com/ChronicNetwork/chtd/issues/31#issuecomment-577058321).
...
Build: `docker build -t ChronicNetwork/chtd:latest .`  or pull from dockerhub
...
docker run --rm -it \
    ...
    ChronicNetwork/chtd:latest /opt/setup_chtd.sh cosmos1pkptre7fdkl6gfrzlesjjvhxhlc3r4gmmk8rs6

# This will start both chtd and rest-server, both are logged
docker run --rm -it -p 26657:26657 -p 26656:26656 -p 1317:1317 \
    --mount type=volume,source=chtd_data,target=/root \
    ChronicNetwork/chtd:latest /opt/run_chtd.sh
```
...
# we need chtd_data volume mount not just for restart, but also to view logs
docker volume rm -f chtd_data
docker run --rm -it -p 26657:26657 -p 26656:26656 -p 9090:9090 \
    --mount type=bind,source=$(pwd)/template,target=/template \
    --mount type=volume,source=chtd_data,target=/root \
    ChronicNetwork/chtd:latest /opt/run_chtd.sh /template
...
* `-X github.com/ChronicNetwork/chtd/app.NodeDir=.corald` - set the config/data directory for the node (default `~/.chtd`)
* `-X github.com/ChronicNetwork/chtd/app.Bech32Prefix=coral` - set the bech32 prefix for all accounts (default `wasm`)
* `-X github.com/ChronicNetwork/chtd/app.ProposalsEnabled=true` - enable all x/wasm governance proposals (default `false`)
* `-X github.com/ChronicNetwork/chtd/app.EnableSpecificProposals=MigrateContract,UpdateAdmin,ClearAdmin` - 
    enable a subset of the x/wasm governance proposal types (overrides `ProposalsEnabled`)

Examples:

* [`chtd`](./Makefile#L50-L55) is a generic, permissionless version using the `cosmos` bech32 prefix
