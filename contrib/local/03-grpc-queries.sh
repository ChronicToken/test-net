...
PROTO_chtd="$DIR/../../proto"
PROTO_chtd_QUERY="$PROTO_chtd/cosmwasm/wasm/v1/query.proto"

echo "### List all codes"
RESP=$(grpcurl -plaintext -import-path "$PROTO_THRD" -import-path "$PROTO_chtd" -proto "$PROTO_chtd_QUERY" \
...
grpcurl -plaintext -import-path "$PROTO_THRD" -import-path "$PROTO_chtd" -proto "$PROTO_chtd_QUERY" \
